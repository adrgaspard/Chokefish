...
static bool run_node_exploration_case(char *fen_string, int32_t depth, uint64_t expected_nodes_count);
...
bool run_node_exploration_batch(char *position_name, char *fen_string, node_exploration_case *cases, uint32_t cases_count)
{
    ...
    for (case_index = 0; case_index < cases_count; case_index++)
    {
        ...
        batch_success &= run_node_exploration_case(fen_string, current_case.depth, current_case.expected_nodes_count);
    }
...
static bool run_node_exploration_case(char *fen_string, int32_t depth, uint64_t expected_nodes_count)
{
    ...
    printf("    ");
    success = expected_nodes_count == found_nodes_count;
    if (success)
    {
        printf(OK_RESULT);
    }
