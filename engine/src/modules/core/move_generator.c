...
static void compute_attack_data(board *board, move_generation_data *data);
static void compute_sliding_attacks(board *board, move_generation_data *data, bitboard pieces_mask, bool ortho_instead_of_diag);
static bool is_position_pinned(bitboard pin_ray_mask, position position);
static void generate_king_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options);
static void generate_sliding_moves(board *board, move_generation_data *data, move_generation_result *result);
static void generate_knight_moves(board *board, move_generation_data *data, move_generation_result *result);
static void generate_pawn_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options);
static void generate_pawn_promotions(position start_pos, position dest_pos, move_generation_result *result, move_generation_options options, bool is_capture);
static bool is_check_after_en_passant(board *board, move_generation_data *data, position start_pos, position dest_pos, position capture_pos);
...
void generate_moves(board *board, move_generation_result *result, move_generation_options options)
{
	...
	// Generate moves
	generate_king_moves(board, &data, result, options);
	if (!data.is_currently_double_check)
	{
		generate_sliding_moves(board, &data, result);
		generate_knight_moves(board, &data, result);
		generate_pawn_moves(board, &data, result, options);
	}
...
static void generate_king_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options)
{
	...
	while (moves_mask != 0)
	{
		...
		result->moves[result->moves_count++] = create_move(data->current_king_pos, dest_pos, is_capture ? MOVE_CAPTURE : MOVE_QUIET);
		assert(is_move_valid(result->moves[result->moves_count - 1]));
		assert(!is_move_empty(result->moves[result->moves_count - 1]));
	}
	...
	if (!data->is_currently_check && options.include_quiet_moves)
	{
		...
		if ((castling_right & CASTLING_KING) != 0)
		{
			...
			if ((castling_mask & castling_blockers_mask) == 0)
			{
				...
				result->moves[result->moves_count++] = create_move(data->current_king_pos, dest_pos, MOVE_KING_CASTLE);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
			}
		}
		if ((castling_right & CASTLING_QUEEN) != 0)
		{
			...
			if ((castling_mask & castling_blockers_mask) == 0 && (castling_block_mask & data->all_pieces_mask) == 0)
			{
				...
				result->moves[result->moves_count++] = create_move(data->current_king_pos, dest_pos, MOVE_QUEEN_CASTLE);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
			}
		}
...
static void generate_sliding_moves(board *board, move_generation_data *data, move_generation_result *result)
{
	...
	while (orthogonal_sliders_mask != 0)
	{
		...
		if (is_position_pinned(data->pin_ray_mask, start_pos))
		{
		...
		while (orthogonal_moves_mask != 0)
		{
			...
			is_capture = (data->enemy_pieces_mask & (1ULL << dest_pos)) != 0;
			result->moves[result->moves_count++] = create_move(start_pos, dest_pos, is_capture ? MOVE_CAPTURE : MOVE_QUIET);
			assert(is_move_valid(result->moves[result->moves_count - 1]));
			assert(!is_move_empty(result->moves[result->moves_count - 1]));
		}
	}
...
static void generate_pawn_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options)
{
	...
	if (options.include_quiet_moves)
	{
		...
		while (no_promotion_single_pushes_mask != 0)
		{
			...
			{
				result->moves[result->moves_count++] = create_move(start_pos, dest_pos, MOVE_QUIET);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
			}
		}
		...
		while (double_pushes_mask != 0)
		{
			...
			if (!is_position_pinned(data->pin_ray_mask, start_pos)
				|| g_align_mask[start_pos][data->current_king_pos] == g_align_mask[dest_pos][data->current_king_pos])
			{
				result->moves[result->moves_count++] = create_move(start_pos, dest_pos, MOVE_DOUBLE_PAWN_PUSH);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
			}
		}
	...
	while (first_capture_mask != 0)
	{
		...
		if (!is_position_pinned(data->pin_ray_mask, start_pos)
			|| g_align_mask[start_pos][data->current_king_pos] == g_align_mask[dest_pos][data->current_king_pos])
		{
			result->moves[result->moves_count++] = create_move(start_pos, dest_pos, MOVE_CAPTURE);
			assert(is_move_valid(result->moves[result->moves_count - 1]));
			assert(!is_move_empty(result->moves[result->moves_count - 1]));
		}
	}
	...
	while (promotion_pushes_mask != 0)
	{
		...
		if (!is_position_pinned(data->pin_ray_mask, start_pos))
		{
			generate_pawn_promotions(start_pos, dest_pos, result, options, false);
		}
	}
	while (first_promotion_capture_mask != 0)
	{
		...
		if (!is_position_pinned(data->pin_ray_mask, start_pos)
			|| g_align_mask[start_pos][data->current_king_pos] == g_align_mask[dest_pos][data->current_king_pos])
		{
			generate_pawn_promotions(start_pos, dest_pos, result, options, true);
		}
	}
		...
		if (contains_position(data->check_ray_mask, en_passant_capture_pos))
		{
			...
			while (en_passant_potential_attackers_mask != 0)
			{
				...
				if (!is_position_pinned(data->pin_ray_mask, start_pos)
					|| g_align_mask[start_pos][data->current_king_pos] == g_align_mask[dest_pos][data->current_king_pos])
				{
					if (!is_check_after_en_passant(board, data, start_pos, dest_pos, en_passant_capture_pos))
					{
						result->moves[result->moves_count++] = create_move(start_pos, dest_pos, MOVE_EN_PASSANT_CAPTURE);
						assert(is_move_valid(result->moves[result->moves_count - 1]));
						assert(!is_move_empty(result->moves[result->moves_count - 1]));
					}
				}
...
static void generate_pawn_promotions(position start_pos, position dest_pos, move_generation_result *result, move_generation_options options, bool is_capture)
{
	...
	assert(result != NULL);
	result->moves[result->moves_count++] = create_move(start_pos, dest_pos, is_capture ? MOVE_QUEEN_PROMOTION_CAPTURE : MOVE_QUEEN_PROMOTION);
	assert(is_move_valid(result->moves[result->moves_count - 1]));
	assert(!is_move_empty(result->moves[result->moves_count - 1]));
	...
	if (options.include_quiet_moves)
	{
		...
		{
			result->moves[result->moves_count++] = create_move(start_pos, dest_pos, is_capture ? MOVE_KNIGHT_PROMOTION_CAPTURE : MOVE_KNIGHT_PROMOTION);
			assert(is_move_valid(result->moves[result->moves_count - 1]));
			assert(!is_move_empty(result->moves[result->moves_count - 1]));
			if (options.promotion_types_to_include == PROMOTION_ALL)
			{
				result->moves[result->moves_count++] = create_move(start_pos, dest_pos, is_capture ? MOVE_ROOK_PROMOTION_CAPTURE : MOVE_ROOK_PROMOTION);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
				result->moves[result->moves_count++] = create_move(start_pos, dest_pos, is_capture ? MOVE_BISHOP_PROMOTION_CAPTURE : MOVE_BISHOP_PROMOTION);
				assert(is_move_valid(result->moves[result->moves_count - 1]));
				assert(!is_move_empty(result->moves[result->moves_count - 1]));
			}
		}
