...
static void _compute_sliding_attacks(board *board, move_generation_data *data, bitboard pieces_mask, bool ortho_instead_of_diag);
static bool _is_position_pinned(bitboard pin_ray_mask, position position);
static void _generate_king_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options);
static void _generate_sliding_moves(board *board, move_generation_data *data, move_generation_result *result);
static void _generate_knight_moves(board *board, move_generation_data *data, move_generation_result *result);
static void _generate_pawn_moves(board *board, move_generation_data *data, move_generation_result *result, move_generation_options options);
static void _generate_pawn_promotions(position start_pos, position dest_pos, move_generation_result *result, move_generation_options options, bool is_capture);
static bool _is_check_after_en_passant(board *board, move_generation_data *data, position start_pos, position dest_pos, position capture_pos);
