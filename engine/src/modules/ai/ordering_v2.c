...
void sort_moves_v2(board *board, move_generation_data *data, move_generation_result *result, move priority_move)
{
    ...
    for (i = 0; i < result->moves_count; i++)
    {
        ...
        if (movement_piece_type != PIECE_PAWN && contains_position(data->opponent_all_attacks_mask, get_dest_pos(result->moves[i])))
        {
            scores[i] -= ATTACKED_SQUARE_PENALTY;
        }
        if (!is_movement_empty(priority_move) && result->moves[i] == priority_move)
        {
            scores[i] += PRIORITY_MOVE_BONUS;
        }
    }
